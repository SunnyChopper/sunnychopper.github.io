...
import type { ParseTaskOutput, TaskBreakdownOutput, PriorityAdvisorOutput, EffortEstimationOutput, TaskCategorizationOutput, DependencyDetectionOutput } from '../../types/llm';
...
export function AITaskAssistPanel({
  ...
  const getModeTitle = () => {
    switch (mode) {
      case 'parse': return 'Smart Parse';
      case 'categorize': return 'Auto-Categorize';
      case 'estimate': return 'Estimate Effort';
      case 'priority': return 'Priority Advisor';
      case 'breakdown': return 'Break Down Task';
      case 'dependencies': return 'Find Dependencies';
    }
  };

  const getModeDescription = () => {
    switch (mode) {
      case 'parse': return 'Describe your task in natural language and AI will structure it for you.';
      case 'categorize': return 'AI will suggest the best area and subcategory for this task.';
      case 'estimate': return 'AI will estimate the effort required based on similar completed tasks.';
      case 'priority': return 'AI will recommend a priority based on your current workload.';
      case 'breakdown': return 'AI will break this task into smaller, actionable subtasks.';
      case 'dependencies': return 'AI will identify tasks this might depend on.';
    }
  };
  ...
  const handleInvoke = () => {
    switch (mode) {
      ...
      case 'categorize': handleCategorize(); break;
      case 'estimate': handleEstimate(); break;
      case 'priority': handlePriority(); break;
      case 'breakdown': handleBreakdown(); break;
      case 'dependencies': handleDetectDependencies(); break;
    }
  };
    ...
    return (
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-800 dark:text-amber-200">AI Not Configured</p>
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-4">
      ...
      {mode !== 'parse' && !isLoading && !error && !categoryResult && !effortResult && !priorityResult && !breakdownResult && !dependencyResult && (
        <Button
          onClick={handleInvoke}
          variant="primary"
          size="sm"
      ...
      {parseResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          ...
          <div className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
            <p><strong>Title:</strong> {parseResult.task.title}</p>
            {parseResult.task.description && <p><strong>Description:</strong> {parseResult.task.description}</p>}
            <p><strong>Area:</strong> {parseResult.task.area}</p>
            {parseResult.task.subCategory && <p><strong>Category:</strong> {parseResult.task.subCategory}</p>}
            <p><strong>Priority:</strong> {parseResult.task.priority}</p>
            {parseResult.task.dueDate && <p><strong>Due:</strong> {parseResult.task.dueDate}</p>}
          </div>
          <div className="flex gap-2 mt-3">
            <Button
              onClick={() => { onApplyParsed?.(parseResult.task); onClose(); }}
              variant="primary"
              size="sm"
            >
      ...
      {categoryResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Suggested Category</span>
            <AIConfidenceIndicator confidence={categoryResult.confidence} size="sm" />
          </div>
          <div className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
            <p><strong>Area:</strong> {categoryResult.area}</p>
            {categoryResult.subCategory && <p><strong>Subcategory:</strong> {categoryResult.subCategory}</p>}
            <p className="text-gray-500 dark:text-gray-400 italic">{categoryResult.reasoning}</p>
          </div>
          <div className="flex gap-2 mt-3">
            <Button
              onClick={() => { onApplyCategory?.(categoryResult.area, categoryResult.subCategory); onClose(); }}
              variant="primary"
              size="sm"
            >
              <Check className="w-4 h-4 mr-1" />
      ...
      {effortResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Effort Estimate</span>
            <AIConfidenceIndicator confidence={effortResult.confidence} size="sm" />
          </div>
          <div className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
            <p><strong>Estimated:</strong> {effortResult.estimatedSize} hours</p>
            {effortResult.comparisons.length > 0 && (
              <ul className="list-disc list-inside text-gray-500 dark:text-gray-400">
                {effortResult.comparisons.map((c, i) => <li key={i}>{c}</li>)}
              </ul>
            )}
          </div>
          <div className="flex gap-2 mt-3">
            <Button
              onClick={() => { onApplyEffort?.(effortResult.estimatedSize); onClose(); }}
              variant="primary"
              size="sm"
            >
              <Check className="w-4 h-4 mr-1" />
      ...
      {priorityResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Priority Recommendation</span>
          </div>
          <div className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
            <p><strong>Recommended:</strong> {priorityResult.recommendedPriority}</p>
            <p className="text-gray-500 dark:text-gray-400 italic">{priorityResult.reasoning}</p>
            <ul className="list-disc list-inside text-gray-500 dark:text-gray-400">
              {priorityResult.factors.map((f, i) => <li key={i}>{f}</li>)}
            </ul>
          </div>
          <div className="flex gap-2 mt-3">
            <Button
              onClick={() => { onApplyPriority?.(priorityResult.recommendedPriority); onClose(); }}
              variant="primary"
              size="sm"
            >
              <Check className="w-4 h-4 mr-1" />
      ...
      {breakdownResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Task Breakdown</span>
          </div>
          <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
            <p className="text-gray-500 dark:text-gray-400 italic mb-2">{breakdownResult.reasoning}</p>
            {breakdownResult.subtasks.map((subtask, i) => (
              <div key={i} className="p-2 bg-gray-50 dark:bg-gray-700 rounded">
                <p className="font-medium">{subtask.title}</p>
                {subtask.description && <p className="text-sm text-gray-500">{subtask.description}</p>}
                <p className="text-xs text-gray-400">{subtask.priority} | {subtask.size ? `${subtask.size}h` : 'Size TBD'}</p>
              </div>
            ))}
          </div>
          <div className="flex gap-2 mt-3">
            <Button
              onClick={() => { onApplyBreakdown?.(breakdownResult.subtasks); onClose(); }}
              variant="primary"
              size="sm"
            >
              <Check className="w-4 h-4 mr-1" />
      ...
      {dependencyResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Suggested Dependencies</span>
          </div>
