...
export function AIProjectAssistPanel({
  ...
  const getModeTitle = () => {
    switch (mode) {
      case 'health': return 'Project Health Analysis';
      case 'generate': return 'Generate Tasks';
  ...
  const getModeDescription = () => {
    switch (mode) {
      case 'health': return 'AI will analyze your project health based on task progress and status.';
      case 'generate': return 'AI will suggest additional tasks needed to complete this project.';
      case 'risks': return 'AI will identify potential risks and blockers for this project.';
    }
  };

  const handleInvoke = () => {
    switch (mode) {
      case 'health': handleAnalyzeHealth(); break;
      case 'generate': handleGenerateTasks(); break;
      case 'risks': handleIdentifyRisks(); break;
    }
  };
    ...
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-4">
      ...
      {healthResult && (
        <div className="mt-4 space-y-4">
          ...
          {healthResult.recommendations.length > 0 && (
            <div className="space-y-2">
              <span className="text-sm font-medium text-gray-900 dark:text-white">Recommendations</span>
      ...
      {tasksResult && (
        <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
          ...
          <div className="flex gap-2 mt-4">
            <Button
              onClick={() => { onCreateTasks?.(tasksResult.suggestedTasks); onClose(); }}
              variant="primary"
              size="sm"
            >
      ...
      {risksResult && (
        <div className="mt-4 space-y-4">
          <div className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-amber-200 dark:border-amber-700">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-gray-900 dark:text-white">Overall Risk Level</span>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRiskColor(risksResult.overallRiskLevel)}`}>
          ...
          {risksResult.risks.length > 0 && (
            <div className="space-y-2">
              <span className="text-sm font-medium text-gray-900 dark:text-white">Identified Risks</span>
