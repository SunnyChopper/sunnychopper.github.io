  ...
  lessons: z
    .array(
      z.object({
        ...
        prerequisites: z
          .array(z.string())
          .describe('Concepts from previous lessons required for this lesson'),
      })
    )
...
export const AlignmentSchema = z.object({
  ...
  issues: z
    .array(
      z.object({
        type: z
          .enum(['gap', 'redundancy', 'prerequisite_missing', 'difficulty_jump'])
...
export const RefinementSchema = z.object({
    ...
    .array(
      z.object({
        ...
        description: z
          .string()
          .nullable()
          .describe('Updated module description, or null if not changed'),
        learningObjectives: z
          .array(z.string())
          .nullable()
          .describe('Updated module learning objectives, or null if not changed'),
      })
    )
    ...
    .array(
      z.object({
        ...
        description: z
          .string()
          .nullable()
          .describe('Updated lesson description, or null if not changed'),
        learningObjectives: z
          .array(z.string())
          .nullable()
          .describe('Updated lesson learning objectives, or null if not changed'),
        keyConcepts: z
          .array(z.string())
          .nullable()
          .describe('Updated key concepts, or null if not changed'),
        prerequisites: z
          .array(z.string())
          .nullable()
          .describe('Updated prerequisites, or null if not changed'),
      })
    )
    ...
    .array(
      z.object({
        ...
        estimatedMinutes: z
          .number()
          .positive()
          .describe('Estimated time in minutes for the new lesson'),
        ...
        insertAfter: z
          .string()
          .nullable()
          .describe('Lesson ID to insert after, or null to append at end'),
      })
    )
  ...
  changes: z
    .array(z.string())
    .describe('Summary descriptions of all changes made during refinement'),
});
  ...
  questions: z
    .array(
      z.object({
        ...
        questionType: z.enum(['yes_no', 'multiple_choice']).describe('Type of question'),
        options: z.array(z.string()).min(2).describe('Answer options for the question'),
      })
    )
    .min(5)
    .max(7)
