...
export const ProgressCoachingOutputSchema = z.object({
  ...
  specificAdvice: z.array(
    z.object({
      area: z.enum(['tasks', 'metrics', 'habits', 'criteria']).describe('Which area this advice targets'),
...
export const GoalDecompositionOutputSchema = z.object({
  subGoals: z.array(
    z.object({
      ...
      timeHorizon: z.enum(['Monthly', 'Quarterly', 'Weekly']).describe('Time horizon for sub-goal'),
...
export const ConflictDetectionOutputSchema = z.object({
  ...
  overcommitmentScore: z.number().min(0).max(100).describe('Overcommitment severity (0=fine, 100=critical)'),
  conflicts: z.array(
    z.object({
      ...
      conflictType: z.enum(['time', 'resource', 'priority', 'values', 'dependencies']).describe('Type of conflict'),
  ...
  capacityAnalysis: z.object({
    ...
    sustainabilityRating: z.enum(['sustainable', 'challenging', 'unsustainable']).describe('Whether workload is sustainable'),
