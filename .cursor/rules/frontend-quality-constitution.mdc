...
- API contracts must be consumed as-is from backend canonical shapes (camelCase)

## API Contract Integrity

- Backend is the source of truth for response shape and casing
- Do not add frontend normalization/mapping to convert snake_case to camelCase
- If API casing/fields drift from contract, fix backend response instead of patching frontend
- Keep service/query caches aligned to one canonical contract shape only
